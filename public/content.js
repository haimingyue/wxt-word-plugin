...
const EXTENSION_PREFS_STORAGE_KEY = 'extraWordPrefsV1';
...
const YT_RT_NEXT_CHUNK_PREFETCH_TRIGGER_OFFSET = 2;
const YT_RT_SHORT_SENTENCE_MAX_CHARS = 16;
const YT_RT_SHORT_SENTENCE_MAX_WORDS = 4;
const YT_RT_SHORT_SENTENCE_MAX_DURATION_SECONDS = 1.1;
const YT_RT_SHORT_SENTENCE_MERGE_GAP_SECONDS = 0.28;
...
const YT_RT_IMMEDIATE_TIMEOUT_MS = 1800;
const YT_RT_IMMEDIATE_RETRY_COOLDOWN_MS = 500;
const YT_RT_PREFETCH_BUFFER_AHEAD_SECONDS = 12;
const YT_RT_PREFETCH_BUFFER_BEHIND_SECONDS = 2;
const YT_RT_PREFETCH_MAX_GROUP_SEGMENTS = 8;
const YT_RT_PREFETCH_MAX_GROUP_CHARS = 900;
const YT_RT_PREFETCH_RETRY_COOLDOWN_MS = 1600;
const YT_RT_BACKFILL_RETRY_LONG_COOLDOWN_MS = 5000;
const YT_RT_REASON_LOG_DEDUP_MS = 1400;
const YT_RT_REASON_LOG_MAX = 1200;
const PAGE_TRANSLATION_VIEWPORT_MARGIN_PX = 360;
const PAGE_TRANSLATION_SCROLL_DEBOUNCE_MS = 180;
...
let selectionMeaningEnabled = true;
let pageTranslationInFlight = false;
let pageTranslationApplied = false;
const pageTranslationRecords = [];
let pageTranslationModeEnabled = false;
let pageTranslationScrollTimer = 0;
let pageTranslationScrollBound = false;
...
let ytRtImmediateLaneInFlight = false;
let ytRtPrefetchLaneInFlight = false;
let ytRtPrefetchTimer = 0;
...
const ytRtTimelineTranslationCache = new Map();
const ytRtTranslationStateByItem = new Map();
const ytRtCaptionTrackCache = new Map();
const ytRtTranslationInflight = new Map();
const ytRtImmediateInflightByItem = new Map();
const ytRtFixedChunkInflight = new Map();
const ytRtFixedChunkLastRequestAt = new Map();
...
let ytRtMetrics = buildEmptyRealtimeMetrics();
...
async function loadExtensionPrefs() {
  try {
    const stored = await chrome.storage.local.get(EXTENSION_PREFS_STORAGE_KEY);
    const prefs = stored?.[EXTENSION_PREFS_STORAGE_KEY];
    selectionMeaningEnabled = prefs?.selectionMeaningEnabled !== false;
  } catch (_) {
    selectionMeaningEnabled = true;
  }
  return {
    selectionMeaningEnabled
  };
}

async function saveExtensionPrefs(partialPrefs = {}) {
  const current = await loadExtensionPrefs();
  const next = {
    ...current,
    ...partialPrefs
  };
  selectionMeaningEnabled = next.selectionMeaningEnabled !== false;
  await chrome.storage.local.set({
    [EXTENSION_PREFS_STORAGE_KEY]: next
  });
  return next;
}
...
function reportTranslateSkip(entry, reasonCode, lane = 'unknown', meta = {}) {
  bumpRealtimeSkipMetric(reasonCode);
...
function buildEmptyRealtimeMetrics() {
  return {
    activeLatencyMs: 0,
    activeResolveMs: 0,
    activeHitLane: '',
    prefetchBufferSeconds: 0,
    inflight: {
      immediate: 0,
      prefetch: 0,
      backfill: 0
    },
    skipCounts: Object.create(null),
    updatedAt: 0
  };
}

function bumpRealtimeSkipMetric(reasonCode) {
  const key = String(reasonCode || 'unknown').trim() || 'unknown';
  ytRtMetrics.skipCounts[key] = Number(ytRtMetrics.skipCounts[key] || 0) + 1;
  ytRtMetrics.updatedAt = Date.now();
}

function updateRealtimeInflightMetrics() {
  ytRtMetrics.inflight = {
    immediate: ytRtImmediateLaneInFlight ? 1 : 0,
    prefetch: ytRtPrefetchLaneInFlight ? 1 : 0,
    backfill: ytRtBackfillInFlight ? 1 : 0
  };
  ytRtMetrics.updatedAt = Date.now();
  try {
    window.__YT_RT_DEBUG__ = {
      ...ytRtMetrics,
      activeItemId: ytRtActiveItemId,
      activeIndex: ytRtActiveIndex,
      transcriptVideoKey: ytRtTranscriptLoadedVideoKey
    };
  } catch (_) {}
}

function buildTimelineTranslationKey(videoKey, item) {
  if (!videoKey || !item) return '';
  const itemId = String(item.id || '').trim();
  const source = String(item.source || '').trim();
  const start = Number(item.videoTimeSeconds || 0).toFixed(3);
  if (!itemId || !source) return '';
  return `${videoKey}|${itemId}|${start}|${source}`;
}

function getTimelineTranslationState(itemId) {
  if (!itemId) return null;
  return ytRtTranslationStateByItem.get(itemId) || null;
}

function setTimelineTranslationState(itemId, partialState = {}) {
  if (!itemId) return null;
  const prev = ytRtTranslationStateByItem.get(itemId) || {
    status: 'idle',
    translation: '',
    lane: '',
    updatedAt: 0,
  ...
  ytRtTranslationStateByItem.set(itemId, next);
  return next;
}

function clearTimelineTranslationState() {
  ytRtTranslationStateByItem.clear();
}

function deleteTimelineTranslationState(itemId) {
  if (!itemId) return;
  ytRtTranslationStateByItem.delete(itemId);
}

function applyTimelineTranslationToItem(videoKey, item, translation, lane = '') {
  if (!item) return false;
  const cleanTranslation = String(translation || '').trim();
  if (!cleanTranslation) return false;
  const source = String(item.source || '').trim();
  const cacheKey = buildTimelineTranslationKey(videoKey, item);
  if (cacheKey) {
    ytRtTimelineTranslationCache.set(cacheKey, cleanTranslation);
    if (ytRtTimelineTranslationCache.size > 4000) {
      const firstKey = ytRtTimelineTranslationCache.keys().next().value;
      if (firstKey) ytRtTimelineTranslationCache.delete(firstKey);
    }
  }
  if (source) {
    ytRtTranslationCache.set(source, cleanTranslation);
  ...
  item.translation = cleanTranslation;
  setTimelineTranslationState(item.id, {
    status: 'translated',
  ...
  ytRtBackfillFailedAt.delete(item.id);
  updateRealtimeItemTranslation(item.id, cleanTranslation);
...
function hydrateTimelineItemTranslation(videoKey, item) {
  if (!item) return false;
  const timelineKey = buildTimelineTranslationKey(videoKey, item);
  const source = String(item.source || '').trim();
  const translated =
    (timelineKey && ytRtTimelineTranslationCache.get(timelineKey)) ||
    (source ? ytRtTranslationCache.get(source) : '');
  if (!translated) {
    setTimelineTranslationState(item.id, {
      status: 'idle',
      translation: '',
      lane: '',
      failedAt: Number(ytRtBackfillFailedAt.get(item.id) || 0)
    });
    return false;
  }
...
function getLaneFailureCooldownMs(lane) {
  const normalized = String(lane || '').trim();
  if (normalized === 'immediate') return YT_RT_IMMEDIATE_RETRY_COOLDOWN_MS;
  if (normalized === 'prefetch') return YT_RT_PREFETCH_RETRY_COOLDOWN_MS;
  return YT_RT_BACKFILL_RETRY_LONG_COOLDOWN_MS;
}

function canRetryTimelineItem(item, lane, { ignoreCooldown = false } = {}) {
  if (!item?.id) return false;
  if (ignoreCooldown) return true;
  const state = getTimelineTranslationState(item.id);
  const failedAt = Number(state?.failedAt || ytRtBackfillFailedAt.get(item.id) || 0);
  if (!failedAt) return true;
  return Date.now() - failedAt >= getLaneFailureCooldownMs(lane);
}

function markTimelineItemFailed(item, lane) {
  if (!item?.id) return;
  const failedAt = Date.now();
  ytRtBackfillFailedAt.set(item.id, failedAt);
  setTimelineTranslationState(item.id, {
    status: 'failed',
    lane,
    failedAt
...
function consumeCachedTranslation(videoKey, item, lane) {
  if (!item) return false;
  const state = getTimelineTranslationState(item.id);
  if (String(state?.translation || '').trim()) {
    item.translation = String(state.translation || '').trim();
    updateRealtimeItemTranslation(item.id, item.translation);
    if (item.id === ytRtActiveItemId) {
      ytRtOverlayPendingItemId = '';
      renderRealtimeSubtitleOverlay(item);
    }
  ...
  const hydrated = hydrateTimelineItemTranslation(videoKey, item);
  if (hydrated) {
    updateRealtimeItemTranslation(item.id, item.translation);
    if (item.id === ytRtActiveItemId) {
      ytRtOverlayPendingItemId = '';
...
function getTranslatedBufferSeconds(centerIndex) {
  if (!Number.isInteger(centerIndex) || centerIndex < 0 || centerIndex >= ytRtItems.length) return 0;
  const center = ytRtItems[centerIndex];
  if (!center) return 0;
  let lastEnd = Number(center.endTimeSeconds || center.videoTimeSeconds || 0);
  for (let index = centerIndex; index < ytRtItems.length; index += 1) {
    const item = ytRtItems[index];
    if (!item) break;
    if (!String(item.translation || '').trim()) break;
    lastEnd = Number(item.endTimeSeconds || item.videoTimeSeconds || lastEnd);
    if (lastEnd - Number(center.videoTimeSeconds || 0) >= YT_RT_PREFETCH_BUFFER_AHEAD_SECONDS) {
      break;
    }
  }
  return Math.max(0, lastEnd - Number(center.videoTimeSeconds || 0));
}
...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  ...
  if (msg?.type === 'extension-get-state') {
    Promise.resolve(loadExtensionPrefs())
      .then((prefs) =>
        sendResponse({
          success: true,
          prefs,
          pageTranslated: pageTranslationModeEnabled,
          pageTranslationInFlight
        })
      )
      .catch((err) => sendResponse({ success: false, message: normalizeExtensionError(err) }));
    return true;
  }

  if (msg?.type === 'extension-set-prefs') {
    Promise.resolve(saveExtensionPrefs(msg.payload || {}))
      .then((prefs) => sendResponse({ success: true, prefs }))
      .catch((err) => sendResponse({ success: false, message: normalizeExtensionError(err) }));
    return true;
  }

  if (msg?.type === 'extension-translate-page') {
    Promise.resolve(translateCurrentPage())
      .then((result) => sendResponse({ success: true, ...result }))
      .catch((err) => sendResponse({ success: false, message: normalizeExtensionError(err) }));
    return true;
  }

  if (msg?.type === 'extension-restore-page') {
    Promise.resolve(restoreTranslatedPage())
      .then((result) => sendResponse({ success: true, ...result }))
      .catch((err) => sendResponse({ success: false, message: normalizeExtensionError(err) }));
    return true;
  }
...
function maybeAutoPopupMeaning() {
  if (!selectionMeaningEnabled) return;
...
function shouldSkipPageTranslateElement(element) {
  ...
  if (
    element.closest?.(
      `#${PANEL_ID},#${MEANING_PANEL_ID},#${YT_RT_PANEL_ID},#${YT_RT_OVERLAY_ID},#${YT_RT_REOPEN_BTN_ID},#${YT_RT_TOGGLE_ID}`
    )
  ) {
    return true;
  }
  if (
    element.closest?.(
      'header,nav,footer,aside,dialog,menu,[role="navigation"],[role="banner"],[role="toolbar"],[role="menubar"],[role="menu"],[role="dialog"],[aria-label*="nav" i],[aria-label*="menu" i]'
    )
  ) {
    return true;
  }
...
function normalizePageTranslationText(text) {
  return String(text || '').replace(/\s+/g, ' ').trim();
}

function getElementOwnTranslatableText(element) {
  if (!element || !(element instanceof HTMLElement)) return '';
  const clone = element.cloneNode(true);
  if (!(clone instanceof HTMLElement)) {
    return normalizePageTranslationText(element.innerText || element.textContent || '');
  }
  clone
    .querySelectorAll('.extra-word-page-translation,.extra-word-page-translation-spinner,[data-extra-word-translated="1"]')
    .forEach((node) => node.remove());
  return normalizePageTranslationText(clone.innerText || clone.textContent || '');
}

function isPageTranslationCandidateElement(element) {
  if (!element || !(element instanceof HTMLElement)) return false;
  if (shouldSkipPageTranslateElement(element)) return false;
  if (element.dataset.extraWordTranslated === '1') return false;
  if (element.dataset.extraWordTranslationPending === '1') return false;
  if (element.querySelector?.('.extra-word-page-translation')) return false;
  if (element.childElementCount > 6) return false;
  const tag = String(element.tagName || '').toLowerCase();
  const allowedTags = ['p', 'li', 'blockquote', 'figcaption', 'summary', 'td', 'th', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6'];
  if (!allowedTags.includes(tag)) return false;
  const text = getElementOwnTranslatableText(element);
...
function collectTranslatableBlocks(root = document.body) {
  if (!root) return [];
  const candidates = Array.from(root.querySelectorAll('p,li,blockquote,figcaption,summary,td,th,h1,h2,h3,h4,h5,h6')).sort(
    (a, b) => getElementDepth(b) - getElementDepth(a)
  );
  const records = [];
  const seen = new Set();
  candidates.forEach((element) => {
    if (!isPageTranslationCandidateElement(element)) return;
    if (records.some((record) => record.element.contains(element) || element.contains(record.element))) return;
    const text = getElementOwnTranslatableText(element);
    if (!text) return;
    const dedupeKey = `${text.slice(0, 120)}|${Math.round(element.getBoundingClientRect().top)}`;
...
function isElementInsideTranslationViewport(element, margin = PAGE_TRANSLATION_VIEWPORT_MARGIN_PX) {
...
function collectViewportTranslatableBlocks(root = document.body) {
  return collectTranslatableBlocks(root).filter((record) =>
    isElementInsideTranslationViewport(record?.element)
  );
}

function buildPageTranslationGroups(records) {
  const groups = [];
  let current = [];
  let currentChars = 0;
  records.forEach((record) => {
    const text = String(record?.normalized || '');
    if (!text) return;
    if (current.length > 0 && (current.length >= 12 || currentChars + text.length > 1200)) {
      groups.push(current);
      current = [];
      currentChars = 0;
    }
...
function ensurePageTranslationSpinner(record) {
  const element = record?.element;
  if (!element || !element.isConnected) return null;
  if (record.spinnerEl?.isConnected) return record.spinnerEl;
  element.dataset.extraWordTranslationPending = '1';
  const spinnerEl = document.createElement('span');
  spinnerEl.className = 'extra-word-page-translation-spinner';
  spinnerEl.setAttribute('aria-hidden', 'true');
  element.appendChild(spinnerEl);
  record.spinnerEl = spinnerEl;
  return spinnerEl;
}

function clearPageTranslationSpinner(record) {
  if (!record) return;
  if (record.element instanceof HTMLElement) {
    delete record.element.dataset.extraWordTranslationPending;
  }
  record.spinnerEl?.remove?.();
  record.spinnerEl = null;
}

function bindPageTranslationViewportListeners() {
  if (pageTranslationScrollBound) return;
  pageTranslationScrollBound = true;
  window.addEventListener('scroll', scheduleViewportTranslationRefresh, { passive: true });
  window.addEventListener('resize', scheduleViewportTranslationRefresh, { passive: true });
}

function unbindPageTranslationViewportListeners() {
  if (!pageTranslationScrollBound) return;
  pageTranslationScrollBound = false;
  window.removeEventListener('scroll', scheduleViewportTranslationRefresh);
  window.removeEventListener('resize', scheduleViewportTranslationRefresh);
}

function scheduleViewportTranslationRefresh() {
  if (!pageTranslationModeEnabled) return;
  if (pageTranslationScrollTimer) {
    clearTimeout(pageTranslationScrollTimer);
  }
  pageTranslationScrollTimer = window.setTimeout(() => {
    pageTranslationScrollTimer = 0;
    if (!pageTranslationModeEnabled || pageTranslationInFlight) return;
    translateVisiblePageBlocks().catch(() => {});
  }, PAGE_TRANSLATION_SCROLL_DEBOUNCE_MS);
}

async function translateVisiblePageBlocks() {
  const root = document.body;
  if (!root) return { pageTranslated: pageTranslationModeEnabled, count: pageTranslationRecords.length };
  const records = collectViewportTranslatableBlocks(root);
  if (!records.length) {
    return { pageTranslated: pageTranslationModeEnabled, count: pageTranslationRecords.length };
  }

  pageTranslationInFlight = true;
  try {
    const groups = buildPageTranslationGroups(records);
    for (const group of groups) {
      group.forEach((record) => ensurePageTranslationSpinner(record));
      const texts = group.map((item) => item.normalized);
      let translated = await requestRealtimeGroupedTranslationWithTimeout(texts, 12000);
      if (translated.length !== texts.length) {
        translated = await requestRealtimeBatchTranslationWithTimeout(texts, 12000);
      }
      if (translated.length !== texts.length) {
        group.forEach((record) => clearPageTranslationSpinner(record));
        continue;
      }
      group.forEach((record, index) => {
        const applied = applyTranslatedTextRecord(record, translated[index]);
        if (applied) pageTranslationRecords.push(record);
      });
    }
    pageTranslationApplied = pageTranslationRecords.length > 0;
    return {
      pageTranslated: pageTranslationModeEnabled,
      count: pageTranslationRecords.length
    };
  } finally {
...
function applyTranslatedTextRecord(record, translatedText) {
  const element = record?.element;
  if (!element || !element.isConnected) return false;
  const translated = String(translatedText || '').trim();
  if (!translated) return false;
  clearPageTranslationSpinner(record);
  const translationEl = document.createElement('div');
  translationEl.className = 'extra-word-page-translation';
  translationEl.textContent = translated;
  element.dataset.extraWordTranslated = '1';
  element.style.overflowWrap = 'anywhere';
  element.style.wordBreak = 'break-word';
  translationEl.style.overflowWrap = 'anywhere';
  translationEl.style.wordBreak = 'break-word';
  element.insertAdjacentElement('afterend', translationEl);
  record.translationEl = translationEl;
  return true;
}
...
async function translateCurrentPage() {
  ...
  if (pageTranslationModeEnabled) {
    return restoreTranslatedPage();
  }

  const root = document.body;
  if (!root) {
    throw new Error('页面主体不可用');
  }
  const records = collectViewportTranslatableBlocks(root);
  if (!records.length) {
    return { pageTranslated: false, count: 0 };
  }
  pageTranslationModeEnabled = true;
  bindPageTranslationViewportListeners();
  return translateVisiblePageBlocks();
}

async function restoreTranslatedPage() {
  pageTranslationRecords.forEach((record) => {
    const element = record?.element;
    if (element instanceof HTMLElement) {
      delete element.dataset.extraWordTranslated;
    }
    clearPageTranslationSpinner(record);
    record?.translationEl?.remove?.();
  });
  pageTranslationRecords.length = 0;
  pageTranslationApplied = false;
  pageTranslationModeEnabled = false;
  pageTranslationInFlight = false;
  if (pageTranslationScrollTimer) {
    clearTimeout(pageTranslationScrollTimer);
    pageTranslationScrollTimer = 0;
  }
  unbindPageTranslationViewportListeners();
  return {
...
function maybeSetupRealtimeSubtitleObserver() {
  ...
  if (routeKey !== ytRtRouteKey) {
    ...
    ytRtImmediateLaneInFlight = false;
    ytRtPrefetchLaneInFlight = false;
    ...
    ytRtBackfillTimer = 0;
    clearTimeout(ytRtPrefetchTimer);
    ytRtPrefetchTimer = 0;
    ytRtOverlayPendingItemId = '';
    ytRtTranslationInflight.clear();
    ytRtImmediateInflightByItem.clear();
    ytRtFixedChunkInflight.clear();
    ytRtFixedChunkLastRequestAt.clear();
    ytRtBackfillFailedAt.clear();
    clearTimelineTranslationState();
    ytRtFullTranscriptInflightByVideo.clear();
    ytRtFullTranscriptKickoffByVideo.clear();
    ytRtSkipReasonLogAt.clear();
    ytRtMetrics = buildEmptyRealtimeMetrics();
    updateRealtimeInflightMetrics();
    ...
    if (ytRtLastVideoPausedState !== paused) {
      ...
      if (!paused) {
        ...
        if (videoKey && hasUntranslatedTimelineItems()) {
          schedulePrefetchLane(videoKey, 0, { centerIndex: ytRtActiveIndex });
          scheduleBackfillLane(videoKey, 240);
        }
      }
    }
    syncRealtimeActiveItemByPlayback(true);
...
function mapSubtitleBundleToRealtimeItems(bundle, videoKey) {
  ...
  return dedupeRealtimeCaptionItems(items);
}

async function applyLoadedRealtimeItems(videoKey, items) {
  ytRtItems = Array.isArray(items) ? items : [];
  ytRtItems.forEach((item) => {
    hydrateTimelineItemTranslation(videoKey, item);
  });
  ytRtStatusText = '';
  ytRtNextLoadAt = 0;
  ...
  ytRtInitialActiveSkipRemaining = Math.max(
    0,
    Math.min(getRealtimeColdStartSkipCount(), ytRtItems.length)
  );
  renderRealtimeSubtitleList();
  syncRealtimeActiveItemByPlayback(true);
  ...
  if (Number.isInteger(ytRtActiveIndex) && ytRtActiveIndex >= 0) {
    triggerActiveSentenceTranslation(videoKey, ytRtActiveIndex);
  }
  schedulePrefetchLane(videoKey, 120);
...
function isLikelyStableCaptionSentence(text) {
  const clean = normalizeCaptionText(text);
  if (!clean) return false;
  if (hasSentenceEnding(clean)) return true;
  if (clean.length >= 28) return true;
  if (countCaptionWords(clean) >= 6) return true;
  return false;
}

function shouldMergeShortCaptionItem(current, next) {
  if (!current || !next) return false;
  const currentSource = normalizeCaptionText(current.source || '');
  const nextSource = normalizeCaptionText(next.source || '');
  if (!currentSource || !nextSource) return false;
  const currentWords = countCaptionWords(currentSource);
  const currentDuration = Math.max(
    0,
    Number(current.endTimeSeconds || current.videoTimeSeconds || 0) -
      Number(current.videoTimeSeconds || 0)
  );
  const gapSeconds = Math.max(
    0,
    Number(next.videoTimeSeconds || 0) - Number(current.endTimeSeconds || current.videoTimeSeconds || 0)
  );
  const isShortSentence =
    currentSource.length <= YT_RT_SHORT_SENTENCE_MAX_CHARS ||
    currentWords <= YT_RT_SHORT_SENTENCE_MAX_WORDS ||
    currentDuration <= YT_RT_SHORT_SENTENCE_MAX_DURATION_SECONDS;
  const looksUnclosed = !hasSentenceEnding(currentSource);
  if (!isShortSentence && !looksUnclosed) return false;
  if (gapSeconds > YT_RT_SHORT_SENTENCE_MERGE_GAP_SECONDS) return false;
  const mergedPreview = normalizeCaptionText(`${currentSource} ${nextSource}`);
  if (mergedPreview.length > 120) return false;
  return true;
}

function mergeShortCaptionItems(items) {
  const normalizedItems = Array.isArray(items) ? items.filter(Boolean) : [];
  if (!normalizedItems.length) return [];
  const merged = [];
  for (const item of normalizedItems) {
    const source = normalizeCaptionText(item.source || '');
    if (!source) continue;
    const normalizedItem = {
      ...item,
      source,
      mergedFromCount: Math.max(1, Number(item.mergedFromCount || 1))
    };
    const previous = merged[merged.length - 1] || null;
    if (previous && shouldMergeShortCaptionItem(previous, normalizedItem)) {
      previous.source = normalizeCaptionText(`${previous.source} ${normalizedItem.source}`);
      previous.endTimeSeconds = Math.max(
        Number(previous.endTimeSeconds || previous.videoTimeSeconds || 0),
        Number(normalizedItem.endTimeSeconds || normalizedItem.videoTimeSeconds || 0)
      );
      previous.mergedFromCount += Math.max(1, Number(normalizedItem.mergedFromCount || 1));
      continue;
    }
...
function trimCaptionBoundaryOverlap(previousText, currentText) {
  const prev = normalizeCaptionText(previousText);
  const curr = normalizeCaptionText(currentText);
  if (!prev || !curr || prev === curr) return curr;

  const prevWords = prev.split(' ').filter(Boolean);
  const currWords = curr.split(' ').filter(Boolean);
  const maxWordOverlap = Math.min(prevWords.length, currWords.length, 10);
  for (let size = maxWordOverlap; size >= 2; size -= 1) {
    ...
    if (prevTail === currHead) {
      const trimmed = currWords.slice(size).join(' ').trim();
      if (trimmed) return trimmed;
    }
  }

  const maxCharOverlap = Math.min(prev.length, curr.length, 80);
  for (let size = maxCharOverlap; size >= 12; size -= 1) {
    const prevTail = prev.slice(-size).toLowerCase();
    const currHead = curr.slice(0, size).toLowerCase();
    if (prevTail !== currHead) continue;
    const before = curr[size - 1] || '';
    const after = curr[size] || '';
    const boundaryLooksSafe =
      /[\s,.;:!?)]/.test(before) || !after || /[\s,.;:!?('"’-]/.test(after);
    if (!boundaryLooksSafe) continue;
    const trimmed = curr.slice(size).trim();
    if (trimmed) return trimmed;
  }
...
function dedupeRealtimeCaptionItems(items) {
  const cleaned = [];
  items.forEach((item) => {
    if (!item) return;
    const previous = cleaned[cleaned.length - 1] || null;
    let source = normalizeCaptionText(item.source || '');
    if (previous?.source) {
      source = trimCaptionBoundaryOverlap(previous.source, source);
    }
    if (!source) return;
    if (
      previous &&
      previous.source === source &&
      Math.abs(Number(item.videoTimeSeconds || 0) - Number(previous.videoTimeSeconds || 0)) <= 1.5
    ) {
      previous.endTimeSeconds = Math.max(
        Number(previous.endTimeSeconds || 0),
        Number(item.endTimeSeconds || 0)
      );
      return;
    }
    cleaned.push({
  ...
  return mergeShortCaptionItems(cleaned).slice(0, YT_RT_MAX_ITEMS);
}
...
function triggerActiveSentenceTranslation(videoKey, index, { isSeek = false } = {}) {
  ...
  if (!ytRtHasFullTimeline) return;
  if (!Number.isInteger(index) || index < 0 || index >= ytRtItems.length) return;
  ytRtHasDispatchedActiveOnce = true;
  ensureImmediateTranslation(videoKey, index, {
    reason: isSeek ? 'seek' : 'playback',
    ignoreCooldown: Boolean(isSeek)
  }).catch((err) => {
    reportTranslateSkip(ytRtItems[index] || null, 'SKIP_CHUNK_NOT_READY', 'immediate', {
      stage: 'ensure_immediate_exception',
      message: normalizeExtensionError(err)
    });
  });
  scheduleActiveTranslationWatchdog(videoKey, index);
  schedulePrefetchLane(videoKey, isSeek ? 0 : 80, {
    centerIndex: index,
    isSeek
  });
}
...
function scheduleActiveTranslationWatchdog(videoKey, index, delayMs = YT_RT_ACTIVE_WATCHDOG_DELAY_MS) {
  ...
  ytRtActiveWatchdogTimer = window.setTimeout(() => {
    ...
    if (ytRtImmediateInflightByItem.has(itemId)) return;
    reportTranslateSkip(latest, 'SKIP_CHUNK_NOT_READY', 'immediate', {
      stage: 'watchdog_force_immediate'
    });
    ensureImmediateTranslation(videoKey, index, {
      reason: 'watchdog',
      ignoreCooldown: true,
...
function schedulePrefetchLane(videoKey, delayMs = 0, options = {}) {
  if (!ytRtEnabled) return;
  if (!videoKey || videoKey !== ytRtTranscriptLoadedVideoKey) return;
  if (!ytRtItems.length) return;
  const centerIndex = Number.isInteger(options?.centerIndex) ? options.centerIndex : ytRtActiveIndex;
  clearTimeout(ytRtPrefetchTimer);
  ytRtPrefetchTimer = window.setTimeout(() => {
    runPrefetchLane(videoKey, {
      centerIndex,
      isSeek: options?.isSeek === true
    }).catch(() => {});
  }, Math.max(0, Number(delayMs) || 0));
}
...
function buildTimelineEntry(index) {
  const item = ytRtItems[index];
  if (!item) return null;
  const source = String(item.source || '').trim();
...
function buildPendingEntriesFromIndices(videoKey, indices, lane, { allowInflight = false, ignoreCooldown = false } = {}) {
  const entries = [];
  const seenSources = new Set();
  indices.forEach((index) => {
    const item = ytRtItems[index];
    if (!item) return;
    if (consumeCachedTranslation(videoKey, item, lane)) {
    ...
    if (seenSources.has(source)) {
      reportTranslateSkip(item, 'SKIP_DUPLICATE', lane, { index, stage: 'pending_indices' });
      return;
    }
    seenSources.add(source);
    if (!allowInflight && ytRtTranslationInflight.has(source)) {
      reportTranslateSkip(item, 'SKIP_IN_FLIGHT', lane, { index });
      return;
    }
    if (!canRetryTimelineItem(item, lane, { ignoreCooldown })) {
      reportTranslateSkip(item, 'SKIP_RATE_LIMIT', lane, { index, stage: 'retry_cooldown' });
      return;
    }
    const entry = buildTimelineEntry(index);
    if (entry) entries.push(entry);
  });
  return entries;
}

function buildTranslationGroups(entries, options = {}) {
  if (!Array.isArray(entries) || !entries.length) return [];
  const targetSegments = Math.max(1, Number(options.targetSegments || YT_RT_BACKFILL_GROUP_TARGET_SEGMENTS) || 1);
  const maxSegments = Math.max(targetSegments, Number(options.maxSegments || targetSegments) || targetSegments);
  const maxChars = Math.max(200, Number(options.maxChars || YT_RT_BACKFILL_GROUP_MAX_CHARS) || 200);
  const maxGapSeconds = Math.max(0.2, Number(options.maxGapSeconds || YT_RT_BACKFILL_GROUP_MAX_GAP_SECONDS) || 0.2);
  const groups = [];
  let current = [];
  let currentChars = 0;
  ...
  for (const entry of entries) {
    ...
    const gapSeconds = previous ? Math.max(0, Number(entry.start || 0) - Number(previous.end || previous.start || 0)) : 0;
    const wouldExceedChars = current.length > 0 && currentChars + source.length > maxChars;
    const wouldExceedSegments = current.length >= maxSegments;
    const wouldBreakGap = current.length > 0 && gapSeconds > maxGapSeconds;
    const reachedTarget = current.length >= targetSegments;
    if (wouldExceedChars || wouldExceedSegments || wouldBreakGap || reachedTarget) {
      pushCurrent();
    }
    current.push(entry);
...
function buildFixedChunkIndices(centerIndex, fixedSize = YT_RT_TRANSLATE_RPC_FIXED_SIZE) {
  ...
  const chunkStart = Math.floor(centerIndex / size) * size;
...
function getFixedChunkStart(centerIndex, fixedSize = YT_RT_TRANSLATE_RPC_FIXED_SIZE) {
  if (!Number.isInteger(centerIndex) || centerIndex < 0 || centerIndex >= ytRtItems.length) return -1;
  const size = Math.max(1, Number(fixedSize) || 1);
  return Math.floor(centerIndex / size) * size;
}

function getIndexOffsetWithinFixedChunk(index, fixedSize = YT_RT_TRANSLATE_RPC_FIXED_SIZE) {
  const chunkStart = getFixedChunkStart(index, fixedSize);
  if (chunkStart < 0) return -1;
  return index - chunkStart;
}

function shouldPrefetchNextFixedChunk(centerIndex, { isSeek = false } = {}) {
  if (isSeek) return true;
  const offset = getIndexOffsetWithinFixedChunk(centerIndex);
  return offset >= Math.max(0, Number(YT_RT_NEXT_CHUNK_PREFETCH_TRIGGER_OFFSET) || 0);
}

function buildNextFixedChunkIndices(centerIndex, fixedSize = YT_RT_TRANSLATE_RPC_FIXED_SIZE) {
  const size = Math.max(1, Number(fixedSize) || 1);
  const currentChunkStart = getFixedChunkStart(centerIndex, size);
  if (currentChunkStart < 0) return [];
  const nextChunkStart = currentChunkStart + size;
  if (nextChunkStart >= ytRtItems.length) return [];
  const nextChunkEnd = Math.min(ytRtItems.length - 1, nextChunkStart + size - 1);
  const indices = [];
  for (let index = nextChunkStart; index <= nextChunkEnd; index += 1) {
    indices.push(index);
  }
  return indices;
}

function getFixedChunkKey(videoKey, entries, fixedSize = YT_RT_TRANSLATE_RPC_FIXED_SIZE) {
  if (!videoKey || !Array.isArray(entries) || !entries.length) return '';
  const size = Math.max(1, Number(fixedSize) || 1);
  const firstIndex = Number(entries[0]?.index);
  if (!Number.isInteger(firstIndex) || firstIndex < 0) return '';
  const chunkStart = Math.floor(firstIndex / size) * size;
  return `${videoKey}:${chunkStart}:${size}`;
}

async function runPrefetchLane(videoKey, { centerIndex = -1, isSeek = false } = {}) {
  if (ytRtPrefetchLaneInFlight) return;
  if (!videoKey || videoKey !== ytRtTranscriptLoadedVideoKey) return;
  if (!ytRtItems.length) return;
  const resolvedCenter = Number.isInteger(centerIndex) && centerIndex >= 0 ? centerIndex : ytRtActiveIndex;
  if (!Number.isInteger(resolvedCenter) || resolvedCenter < 0 || resolvedCenter >= ytRtItems.length) return;

  ytRtMetrics.prefetchBufferSeconds = getTranslatedBufferSeconds(resolvedCenter);
  ytRtMetrics.updatedAt = Date.now();
  if (!shouldPrefetchNextFixedChunk(resolvedCenter, { isSeek })) {
    return;
  }

  const indices = buildNextFixedChunkIndices(resolvedCenter);
  const entries = buildPendingEntriesFromIndices(videoKey, indices, 'prefetch');
  if (!entries.length) {
    if (ytRtMetrics.prefetchBufferSeconds >= Math.max(6, YT_RT_PREFETCH_BUFFER_AHEAD_SECONDS - 2)) {
      scheduleBackfillLane(videoKey, YT_RT_BACKFILL_IDLE_DELAY_MS);
    }
    return;
  }

  ytRtPrefetchLaneInFlight = true;
  updateRealtimeInflightMetrics();
  try {
    const groups = buildTranslationGroups(entries, {
      targetSegments: Math.max(1, Number(YT_RT_TRANSLATE_RPC_FIXED_SIZE) || 1),
      maxSegments: Math.max(1, Number(YT_RT_TRANSLATE_RPC_FIXED_SIZE) || 1),
      maxChars: YT_RT_PREFETCH_MAX_GROUP_CHARS,
      maxGapSeconds: 2.6
    });
    for (const group of groups) {
      if (videoKey !== ytRtTranscriptLoadedVideoKey) return;
      await translateEntryGroup(videoKey, group, {
        timeoutMs: Math.min(YT_RT_ACTIVE_GROUP_TIMEOUT_MS, 4200),
        allowBatchFallback: true,
        lane: 'prefetch',
        batchMode: 'fixed'
      });
      if (ytRtMetrics.prefetchBufferSeconds >= YT_RT_PREFETCH_BUFFER_AHEAD_SECONDS) {
        break;
      }
    }
  } finally {
    ...
    if (videoKey === ytRtTranscriptLoadedVideoKey && hasUntranslatedTimelineItems()) {
      if (shouldPrefetchNextFixedChunk(ytRtActiveIndex, { isSeek: false })) {
        schedulePrefetchLane(videoKey, 220, { centerIndex: ytRtActiveIndex, isSeek: false });
      } else {
        scheduleBackfillLane(videoKey, 400);
      }
    }
  }
}
...
async function runBackfillLane(videoKey) {
  ...
  if (videoKey !== ytRtTranscriptLoadedVideoKey) return;
  if (isCurrentVideoPaused()) return;
  if (ytRtImmediateLaneInFlight || ytRtPrefetchLaneInFlight) {
    scheduleBackfillLane(videoKey, 600);
    return;
  }
  const activeIndex = Number.isInteger(ytRtActiveIndex) ? ytRtActiveIndex : 0;
  ytRtMetrics.prefetchBufferSeconds = getTranslatedBufferSeconds(activeIndex);
  if (ytRtMetrics.prefetchBufferSeconds < Math.max(6, YT_RT_PREFETCH_BUFFER_AHEAD_SECONDS - 2)) {
    schedulePrefetchLane(videoKey, 120, { centerIndex: activeIndex });
    return;
  }

  const fixedBatchSize = Math.max(1, Number(YT_RT_TRANSLATE_RPC_FIXED_SIZE) || 1);
  let targetLimit = Math.max(1, Number(YT_RT_BACKFILL_STEP_BATCH) || fixedBatchSize);
  if (isBackfillTimesliceEnabled()) {
    const grantedBatches = consumeBackfillTimesliceBatches(1);
    if (grantedBatches <= 0) {
      reportTranslateSkip({ id: 'backfill-timeslice' }, 'SKIP_RATE_LIMIT', 'backfill', {
        stage: 'timeslice_budget_empty'
      });
      scheduleBackfillLane(videoKey, Math.max(YT_RT_BACKFILL_IDLE_DELAY_MS, Math.floor(YT_RT_BACKFILL_TIMESLICE_INTERVAL_MS / 2)));
      return;
    }
    targetLimit = grantedBatches * fixedBatchSize;
  }

  const targets = collectBackfillTargets(targetLimit, activeIndex);
  if (!targets.length) return;
  const entries = buildPendingEntriesFromIndices(videoKey, targets, 'backfill');
  ...
  try {
    const groups = buildTranslationGroups(entries, {
      targetSegments: Math.max(1, Number(YT_RT_TRANSLATE_RPC_FIXED_SIZE) || 1),
      maxSegments: Math.max(1, Number(YT_RT_TRANSLATE_RPC_FIXED_SIZE) || 1),
      maxChars: YT_RT_BACKFILL_GROUP_MAX_CHARS,
      maxGapSeconds: YT_RT_BACKFILL_GROUP_MAX_GAP_SECONDS
    });
    for (const group of groups) {
      if (videoKey !== ytRtTranscriptLoadedVideoKey) return;
      await translateEntryGroup(videoKey, group, {
...
function markEntriesFailed(entries, lane = 'backfill') {
  for (const entry of entries) {
    const item = ytRtItems[Number(entry?.index)];
    if (!item || String(item.id || '') !== String(entry?.id || '')) continue;
    markTimelineItemFailed(item, lane);
  }
}
...
async function translateEntryGroup(
  ...
  const effectiveMode = String(batchMode || '').toLowerCase() === 'fixed' ? 'fixed' : 'adaptive';
  const fixedChunkKey = effectiveMode === 'fixed' ? getFixedChunkKey(videoKey, dedupedEntries, fixedSize) : '';
  if (fixedChunkKey) {
    const existingChunkTask = ytRtFixedChunkInflight.get(fixedChunkKey);
    if (existingChunkTask) {
      await existingChunkTask;
      return true;
    }
    const lastAt = Number(ytRtFixedChunkLastRequestAt.get(fixedChunkKey) || 0);
    if (lastAt > 0 && Date.now() - lastAt < 900) {
      reportTranslateSkipBatch(dedupedEntries, 'SKIP_IN_FLIGHT', lane, {
        stage: 'fixed_chunk_recently_requested',
        chunkKey: fixedChunkKey
      });
      return true;
    }
  }
  ...
  const runTask = async () => {
    ...
    for (const entry of dedupedEntries) {
      ...
      const item = ytRtItems[Number(entry.index)];
      if (item && String(item.id || '') === String(entry.id || '')) {
        setTimelineTranslationState(item.id, {
          status: lane === 'prefetch' ? 'inflight_prefetch' : lane === 'immediate' ? 'inflight_immediate' : 'inflight_backfill',
          lane,
    ...
    const groups =
      effectiveMode === 'fixed'
        ? buildTranslationGroups(sortedEntries, {
            targetSegments: fixedSize,
            maxSegments: fixedSize,
            maxChars: YT_RT_BACKFILL_GROUP_MAX_CHARS,
            maxGapSeconds: YT_RT_BACKFILL_GROUP_MAX_GAP_SECONDS
          })
        : chunkArray(sortedEntries, Math.min(5, Math.max(1, Number(adaptiveBatchSize) || 1)));

    for (const chunkEntries of groups) {
      const payload =
        effectiveMode === 'fixed'
          ? buildFixedTranslatePayload(chunkEntries, fixedSize)
          : {
              sources: chunkEntries.map((entry) => String(entry?.source || '').trim()).filter(Boolean),
              targetCount: chunkEntries.length
            };
      const sources = payload.sources;
      if (!sources.length) {
        reportTranslateSkipBatch(chunkEntries, 'SKIP_NOT_ENOUGH_TO_BUILD_PAYLOAD', lane, {
          stage: `${effectiveMode}_empty_sources`
        });
        markEntriesFailed(chunkEntries, lane);
        allSucceeded = false;
        continue;
      }
      ...
      reportTranslateDispatch(lane, sources, {
        ...
        fixedSize: effectiveMode === 'fixed' ? sources.length : undefined,
      ...
      if (translations.length === sources.length) {
        applyBackfillTranslations(
          videoKey,
          chunkEntries,
          effectiveMode === 'fixed' ? translations.slice(0, payload.targetCount) : translations,
          lane
        );
        continue;
      }
  ...
  const task = runTask();
  if (fixedChunkKey) {
    ytRtFixedChunkLastRequestAt.set(fixedChunkKey, Date.now());
    ytRtFixedChunkInflight.set(fixedChunkKey, task);
  }
  try {
    return await task;
  } finally {
    if (fixedChunkKey && ytRtFixedChunkInflight.get(fixedChunkKey) === task) {
      ytRtFixedChunkInflight.delete(fixedChunkKey);
    }
  }
}

function applyBackfillTranslations(videoKey, entries, translations, lane = 'backfill') {
  for (let i = 0; i < entries.length; i += 1) {
    ...
    applyTimelineTranslationToItem(videoKey, item, translated, lane);
    if (lane === 'immediate') {
      ytRtMetrics.activeResolveMs = Math.max(
        0,
        Date.now() - Number(getTimelineTranslationState(item.id)?.requestedAt || Date.now())
      );
      ytRtMetrics.activeHitLane = 'immediate';
      ytRtMetrics.updatedAt = Date.now();
    }
  }
}

function collectBackfillTargets(maxCount = YT_RT_BACKFILL_STEP_BATCH, activeIndex = -1) {
  ...
  for (let step = 0; step < total && targets.length < limit; step += 1) {
    ...
    const source = String(item.source || '').trim();
    if (!source) continue;
    const activeItem = Number.isInteger(activeIndex) ? ytRtItems[activeIndex] : null;
    if (activeItem && Number(item.videoTimeSeconds || 0) >= Number(activeItem.videoTimeSeconds || 0)) {
      continue;
    }
    if (ytRtTranslationInflight.has(source)) continue;
    if (!canRetryTimelineItem(item, 'backfill')) continue;
    targets.push(index);
  }
...
function hasUntranslatedTimelineItems() {
  return ytRtItems.some((item) => item && !String(item.translation || '').trim());
}
...
async function ensureImmediateTranslation(
  videoKey,
  index,
  { reason = 'playback', ignoreCooldown = false, force = false } = {}
) {
  const item = ytRtItems[index];
  if (!item) return false;
  if (videoKey !== ytRtTranscriptLoadedVideoKey) return false;
  if (consumeCachedTranslation(videoKey, item, 'immediate')) {
    ytRtMetrics.activeHitLane = 'cache';
    ytRtMetrics.updatedAt = Date.now();
    return true;
  }
  if (!canRetryTimelineItem(item, 'immediate', { ignoreCooldown: ignoreCooldown || force })) {
    reportTranslateSkip(item, 'SKIP_RATE_LIMIT', 'immediate', {
      stage: 'immediate_retry_cooldown',
      reason
    });
    return false;
  }
  ...
  const existing = ytRtImmediateInflightByItem.get(itemId);
  if (existing) return existing;

  const requestedAt = Date.now();
  ytRtImmediateLaneInFlight = true;
  updateRealtimeInflightMetrics();
  ytRtMetrics.activeLatencyMs = Math.max(0, requestedAt - Number(item.createdAt || requestedAt));
  ytRtMetrics.activeHitLane = 'immediate';
  ytRtMetrics.updatedAt = requestedAt;
  setTimelineTranslationState(itemId, {
    status: 'inflight_immediate',
    lane: 'immediate',
    requestedAt,
    resolvedAt: 0
  });
  ...
  const task = (async () => {
    const fixedChunkIndices = buildFixedChunkIndices(index);
    const fixedChunkEntries = buildPendingEntriesFromIndices(videoKey, fixedChunkIndices, 'immediate', {
      allowInflight: true,
      ignoreCooldown: true
    });
    if (fixedChunkEntries.length) {
      const groupedOk = await translateEntryGroup(videoKey, fixedChunkEntries, {
        timeoutMs: Math.max(YT_RT_IMMEDIATE_TIMEOUT_MS, 2200),
        allowBatchFallback: true,
        lane: 'immediate',
        batchMode: 'fixed'
      });
      const refreshedItem = ytRtItems[index];
      if (groupedOk && String(refreshedItem?.translation || '').trim()) {
        return true;
      }
    }
    markTimelineItemFailed(item, 'immediate');
    if (item.id === ytRtActiveItemId) {
      renderRealtimeSubtitleOverlay(item);
    }
    return false;
  })().finally(() => {
    ytRtImmediateInflightByItem.delete(itemId);
    ytRtImmediateLaneInFlight = ytRtImmediateInflightByItem.size > 0;
    updateRealtimeInflightMetrics();
  });

  ytRtImmediateInflightByItem.set(itemId, task);
  return task;
}
...
function pushRealtimeSourceOnlyItem(source, videoTimeSeconds = -1) {
  ...
  const lastItem = ytRtItems[ytRtItems.length - 1] || null;
  const previousSource = String(lastItem?.source || '').trim();
  const sourceTime = Number.isFinite(videoTimeSeconds) ? Number(videoTimeSeconds) : -1;
  const previousTime = Number(lastItem?.videoTimeSeconds || -1);
  const isIncrementalCaptionUpdate =
    lastItem &&
    previousSource &&
    sourceTime >= 0 &&
    previousTime >= 0 &&
    Math.abs(sourceTime - previousTime) <= 2.2 &&
    (cleanSource.startsWith(previousSource) ||
      previousSource.startsWith(cleanSource) ||
      normalizeCaptionText(cleanSource).includes(normalizeCaptionText(previousSource)));

  if (isIncrementalCaptionUpdate) {
    lastItem.source = cleanSource;
    lastItem.translation = '';
    lastItem.videoTimeSeconds = sourceTime;
    deleteTimelineTranslationState(lastItem.id);
    setTimelineTranslationState(lastItem.id, {
      status: 'idle',
      translation: '',
      lane: ''
    });
    if (lastItem.id === ytRtActiveItemId) {
      renderRealtimeSubtitleOverlay(lastItem);
    }
    renderRealtimeSubtitleList();
  ...
  if (lastItem && previousSource && !isLikelyStableCaptionSentence(previousSource)) {
    lastItem.source = normalizeCaptionText(`${previousSource} ${cleanSource}`);
    lastItem.translation = '';
    if (sourceTime >= 0) {
      lastItem.videoTimeSeconds = previousTime >= 0 ? previousTime : sourceTime;
      lastItem.endTimeSeconds = sourceTime;
    }
    lastItem.mergedFromCount = Math.max(1, Number(lastItem.mergedFromCount || 1)) + 1;
    deleteTimelineTranslationState(lastItem.id);
    setTimelineTranslationState(lastItem.id, {
      status: 'idle',
      translation: '',
  ...
  if (previousSource) {
    cleanSource = trimCaptionBoundaryOverlap(previousSource, cleanSource);
  }
  if (!cleanSource) return;
  const canMergeWithLast =
    ytRtItems.length &&
  ...
  if (canMergeWithLast) {
    ...
    ytRtItems.push({
      ...
      translation: '',
      videoTimeSeconds: Number.isFinite(videoTimeSeconds) ? videoTimeSeconds : -1,
      endTimeSeconds: Number.isFinite(videoTimeSeconds) ? videoTimeSeconds : -1,
      mergedFromCount: 1,
      createdAt: Date.now()
    });
    if (ytRtItems.length > YT_RT_MAX_ITEMS) {
    ...
    const created = ytRtItems[ytRtItems.length - 1];
    if (created) {
      setTimelineTranslationState(created.id, {
        status: 'idle',
        translation: '',
        lane: ''
      });
  ...
  if (ytRtActiveIndex < 0 && ytRtItems.length) {
    ...
    if (videoKey && ytRtHasFullTimeline) {
      triggerActiveSentenceTranslation(videoKey, fallbackIndex, { isSeek: false });
    }
  }
  const videoKey = String(ytRtTranscriptLoadedVideoKey || getCurrentYouTubeVideoKey() || '').trim();
  if (videoKey && ytRtHasFullTimeline) {
    schedulePrefetchLane(videoKey, 120, { centerIndex: ytRtActiveIndex });
  }
}
